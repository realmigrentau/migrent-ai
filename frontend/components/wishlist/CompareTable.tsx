    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-4xl max-h-[85vh] bg-[var(--color-surface-2)] rounded-2xl shadow-2xl overflow-hidden flex flex-col"
        >
          {/* Header */}
          ...
          <div className="flex-1 overflow-auto">
            <div className="min-w-[600px]">
              {/* Photo row */}
              <div className="grid border-b border-[var(--color-line)]" style={{ gridTemplateColumns: `120px repeat(${listings.length}, 1fr)` }}>
                <div className="p-3 flex items-center">
                  <span className="text-xs font-semibold text-[var(--color-ink-3)]">Photo</span>
                </div>
                {listings.map((l) => (
                  <div key={l.id} className="p-3 relative group">
                    <div className="aspect-[4/3] rounded-xl overflow-hidden bg-[var(--color-surface-muted)]">
                      <img
                        src={l.photos[0]}
                        alt={l.address}
                    ...
                    <button
                      ...
                      className="absolute top-4 right-4 p-1 rounded-full bg-white/80 dark:bg-[var(--color-surface)]/80 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      <X className="w-3 h-3 text-[var(--color-ink-3)]" />
                    </button>
                  </div>
                ))}
              </div>
              ...
              <CompareRow label="Address" listings={listings}>
                {(l) => (
                  <div>
                    <p className="text-sm font-bold text-[var(--color-ink)]">{l.address}</p>
                    <p className="text-xs text-[var(--color-ink-3)]">{l.suburb}, {l.postcode}</p>
                  </div>
                )}
              </CompareRow>
                ...
                {(l) => (
                  <div className="flex items-center gap-1.5">
                    <Star className={`w-4 h-4 ${
                      l.rating === highestRating ? "fill-amber-400 text-[var(--color-warn-500)]" : "fill-slate-300 text-[var(--color-ink-4)] dark:fill-slate-600 dark:text-[var(--color-ink-2)]"
                    }`} />
                    <span className={`text-sm font-bold ${
                      l.rating === highestRating ? "text-[var(--color-warn-500)]" : "text-[var(--color-ink-2)]"
                    }`}>
                      {l.rating}
                    </span>
                ...
                {(l) => (
                  <div className="flex items-center gap-1.5">
                    <Train className={`w-3.5 h-3.5 ${
                      l.stationWalkMin === closestStation ? "text-[var(--color-primary)]" : "text-[var(--color-ink-3)]"
                    }`} />
                    <span className={`text-sm ${
                      l.stationWalkMin === closestStation ? "font-bold text-[var(--color-primary)]" : "text-[var(--color-ink-2)]"
                    }`}>
                      {l.stationWalkMin ? `${l.stationWalkMin}min` : "N/A"}
                    </span>
                    {l.nearestStation && (
                      <span className="text-xs text-[var(--color-ink-3)]">{l.nearestStation}</span>
                    )}
                  </div>
                )}
              </CompareRow>
              ...
              <CompareRow label="Room" listings={listings}>
                {(l) => (
                  <span className="text-sm capitalize text-[var(--color-ink-2)]">{l.roomType}</span>
                )}
              </CompareRow>
              ...
              <CompareRow label="Amenities" listings={listings}>
                {(l) => (
                  <div className="flex flex-wrap gap-1">
                    {l.furnished && (
                      <span className="px-1.5 py-0.5 rounded text-[10px] bg-[var(--color-surface-muted)] text-[var(--color-ink-3)]">Furnished</span>
                    )}
                    {l.billsIncluded && (
                      <span className="px-1.5 py-0.5 rounded text-[10px] bg-[var(--color-surface-muted)] text-[var(--color-ink-3)]">Bills incl.</span>
                    )}
                  </div>
                )}
              </CompareRow>
                ...
                {(l) => (
                  <div className="flex items-center gap-1">
                    {l.verified ? (
                      ...
                      <span className="text-xs text-[var(--color-ink-3)]">No</span>
                    )}
                  </div>
                )}
              </CompareRow>
              ...
              <CompareRow label="Host" listings={listings}>
                {(l) => (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm text-[var(--color-ink-2)]">{l.ownerName}</span>
                    {l.superhost && <Sparkles className="w-3 h-3 text-[var(--color-warn-500)]" />}
                  </div>
                )}
              </CompareRow>
              ...
              <CompareRow label="Reply" listings={listings}>
                {(l) => l.ownerReply ? (
                  ...
                  <span className="text-xs text-[var(--color-ink-4)]">No reply yet</span>
                )}
              </CompareRow>
            </div>
          </div>
          ...
          {/* Actions */}
          <div className="px-5 py-4 border-t border-[var(--color-line)] flex items-center gap-3">
            <button
              onClick={onClose}
...
function CompareRow({
  ...
  return (
    <div
      className={`grid border-b border-[var(--color-line)]/50 dark:border-slate-800/50 ${
        highlight ? "bg-[var(--color-surface)]/50 dark:bg-[var(--color-surface-muted)]/20" : ""
      }`}
      style={{ gridTemplateColumns: `120px repeat(${listings.length}, 1fr)` }}
    >
      <div className="p-3 flex items-center">
        <span className="text-xs font-semibold text-[var(--color-ink-3)]">{label}</span>
      </div>
      {listings.map((l) => (
