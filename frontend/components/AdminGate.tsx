...
export default function AdminGate({ children }: { children: ReactNode }) {
  ...
  const [lockReason, setLockReason] = useState<LockReason>("attempts");
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-[var(--color-danger-600)]">
          ...
          >
            ...
            <p className="text-white/70 text-sm mb-6">
              {isInactivity
                ? "You have been locked out due to our secure protocols."
                : "Too many failed attempts. Enter recovery password to continue."}
            </p>
            <input
              type="password"
              placeholder={isInactivity ? "Enter password to continue" : "Recovery password"}
    ...
    <div className="min-h-[70vh] flex items-center justify-center px-4">
      ...
      >
        <div className="card p-8 rounded-2xl">
          <div className="absolute top-0 left-0 right-0 h-px bg-[var(--color-primary)] from-transparent via-primary-soft to-transparent rounded-t-2xl" />
