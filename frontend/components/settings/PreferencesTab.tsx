    ...
    <div className="space-y-6">
      ...
      <GlassCard delay={0.15}>
        <div className="flex items-start gap-4 mb-4">
          ...
          <div>
            <h3 className="font-bold text-[var(--color-ink)]">Region & Timezone</h3>
            <p className="text-xs text-[var(--color-ink-3)]">Set your local preferences</p>
          </div>
        </div>

        <div className="ml-14 space-y-4">
          <div>
            <label className="block text-xs font-medium text-[var(--color-ink-3)] mb-2">Region</label>
                ...
                <button
                  ...
                  className={`
                    ...
                    ${region === r.value
                      ...
                      : "border-[var(--color-line)] text-[var(--color-ink-2)] hover:border-[var(--color-line-2)]"
                    }
                  `}
                >
                  {r.emoji} {r.label}
          ...
          <div>
            <label className="block text-xs font-medium text-[var(--color-ink-3)] mb-2">Timezone</label>
            <select
              value={timezone}
          ...
          <div>
            <label className="block text-xs font-medium text-[var(--color-ink-3)] mb-2">Currency</label>
            <select value="AUD" disabled className="input-field bg-[var(--color-surface)] cursor-not-allowed">
              <option>AUD (Australian Dollar)</option>
            </select>
            <p className="text-[10px] text-[var(--color-ink-3)] mt-1">All transactions are in AUD</p>
          </div>
        </div>
      </GlassCard>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-bold text-[var(--color-ink)]">Keyboard Shortcuts</h3>
                <p className="text-xs text-[var(--color-ink-3)]">Navigate faster with hotkeys</p>
              </div>
              <button
            ...
            >
              <div className="ml-14 mt-4 space-y-2">
                {shortcuts.map((shortcut, i) => (
                  ...
                  >
                    <span className="text-sm text-[var(--color-ink-2)]">{shortcut.action}</span>
                    <div className="flex items-center gap-1">
                      {shortcut.keys.map((key, j) => (
                        <kbd
                          key={j}
                          className="px-2 py-0.5 rounded-md bg-[var(--color-surface-muted)] border border-[var(--color-line)] text-[11px] font-mono text-[var(--color-ink-2)] shadow-sm"
                        >
                          {key}
                        </kbd>
                      ))}
