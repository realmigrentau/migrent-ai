...
export default function ReviewCarousel({ reviews, reviewsCount, averageRating, loading, ownerName }: ReviewCarouselProps) {
      ...
      <div>
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="flex items-center gap-1">
            <svg className="w-5 h-5 text-[var(--color-warn-500)]" viewBox="0 0 24 24" fill="currentColor">
              ...
              <div className="card p-5 rounded-2xl h-full">
                ...
                <div className="flex gap-0.5 mb-3">
                  {Array.from({ length: 5 }).map((_, s) => (
                    <svg
                      key={s}
                      className={`w-4 h-4 ${s < review.rating ? "text-[var(--color-warn-500)]" : "text-[var(--color-ink-4)] dark:text-[var(--color-ink-2)]"}`}
                      viewBox="0 0 24 24"
                      fill="currentColor"
                    >
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                ...
                {/* Comment */}
                <p className="text-sm text-[var(--color-ink-2)] leading-relaxed line-clamp-4">
                ...
                <div className="flex items-center gap-3 mt-4 pt-3 border-t border-[var(--color-line)]">
                  <div className="w-8 h-8 rounded-full bg-[var(--color-primary-soft)] from-[var(--color-primary)] to-[var(--color-primary)] flex items-center justify-center overflow-hidden">
                    {review.reviewer_photo ? (
                      <img src={review.reviewer_photo} alt={`${review.reviewer_name || "Reviewer"} profile photo`} className="w-full h-full object-cover" />
                    ) : (
                      <span className="text-xs font-bold text-white">{review.reviewer_name.charAt(0)}</span>
                    )}
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-[var(--color-ink)]">{review.reviewer_name}</p>
            ...
            {reviews.slice(0, Math.min(reviews.length, 6)).map((_, i) => (
              <div
                ...
                className={`w-1.5 h-1.5 rounded-full transition-colors ${
                  i === currentIndex ? "bg-[var(--color-primary)]" : "bg-[var(--color-line)]"
                }`}
              />
            ))}
          </div>
  ...
  return (
    <div className="card-subtle p-8 rounded-2xl text-center">
      ...
      <p className="text-sm font-medium text-[var(--color-ink-2)]">No reviews yet</p>
      <p className="text-xs text-[var(--color-ink-3)] mt-1">Reviews appear after completed stays</p>
    </div>
  );
}
