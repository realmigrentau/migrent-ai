...
export default function AgentTicketPage() {
  ...
  if (error || !ticket) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/support/agent" className="text-rose-500 hover:text-rose-600 font-medium text-sm">
          Back to dashboard
        </Link>
      </div>
    );
  ...
  return (
    <>
      <Head>
        <title>Ticket {ticket.id.slice(0, 8)} - Agent Dashboard</title>
      </Head>

      <div className="max-w-4xl mx-auto">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-sm text-slate-400 mb-6">
          <Link href="/support/agent" className="hover:text-rose-500 transition-colors">Agent Dashboard</Link>
          <span>/</span>
