    ...
    <DashboardLayout>
      <div className="max-w-3xl mx-auto">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                showUnreadOnly
                  ...
                  : "bg-[var(--color-surface-muted)] text-[var(--color-ink-3)] hover:bg-[var(--color-surface-muted)]"
              }`}
            >
        ...
        <motion.div
          ...
          className="bg-[var(--color-surface-2)] border border-[var(--color-line)]/70 dark:border-slate-800 rounded-2xl overflow-hidden"
        >
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16">
              <Loader2 className="w-6 h-6 text-[var(--color-ink-3)] animate-spin mb-3" />
              <p className="text-sm text-[var(--color-ink-3)]">Loading notifications...</p>
            </div>
          ) : notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 px-6">
              <div className="w-16 h-16 rounded-full bg-[var(--color-surface-muted)] flex items-center justify-center mb-4">
                <BellOff className="w-7 h-7 text-[var(--color-ink-3)]" />
              </div>
              <p className="text-base font-medium text-[var(--color-ink-2)] mb-1">
                {showUnreadOnly ? "No unread notifications" : "No notifications yet"}
              </p>
              <p className="text-sm text-[var(--color-ink-3)] text-center max-w-xs">
                {showUnreadOnly
                      ...
                      <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        {!n.is_read && (
                          <button
                            ...
                            title="Mark as read"
                            className="p-1.5 rounded-lg text-[var(--color-ink-3)] hover:text-[var(--color-primary)] hover:bg-[var(--color-primary-soft)] dark:hover:bg-[var(--color-primary)]/10 transition-colors"
                          >
